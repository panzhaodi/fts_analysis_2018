...
'''
This code plot the raw spectra without normalizing. 
'''
wafer=wafer[8]

#remove darks, saturated ones, and other bad ones. 
...
for data_file in data_90ghz:
 if filter_spectra(data_file):
  try:
      ...
      pl.plot(data['freq'],np.array(data['real']) ,color='b',lw=0.5) 
  except EOFError:
      print data_file
...
for data_file in data_90ghz:
 if filter_spectra(data_file):
  try:
      ...
      data['freq']=data['freq'][np.where(data['freq']<300)]
      data['real']=data['real'][np.where(data['freq']<300)]
      pl.plot(data['freq'],np.array(data['real']) ,color='g',lw=0.5) 
  except EOFError:
      print data_file
for data_file in data_150ghz:
 if filter_spectra(data_file):
...
for data_file in data_90ghz:
 if filter_spectra(data_file):
  try:
      ...
      data['freq']=data['freq'][np.where(data['freq']<300)]
      data['real']=data['real'][np.where(data['freq']<300)]
      pl.plot(data['freq'],np.array(data['real']) ,color='r',lw=0.5) 
  except EOFError:
      print data_file
for data_file in data_150ghz:
 if filter_spectra(data_file):
...
pl.axis([50,300, 0, 60000])
pl.legend(loc=2)
pl.grid()
pl.title(wafer+ ' raw spectra, not corrected, unnormalized')
#
pl.xlabel('Frequency(GHz)')
pl.ylabel('Arbitrary Unit')
pl.savefig(wafer+'raw_spectra_unnormalized')
